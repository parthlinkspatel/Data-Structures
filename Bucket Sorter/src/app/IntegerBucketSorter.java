...
public class IntegerBucketSorter implements Sorter {
   ...
   */
   public int[] sort(int[] dataArray) throws Exception {
      ...
      resetBucketValues();
      int maxInt = findMaxIntLength(dataArray);
   ...
   public void distribute(int[] dataArray, int curPlace){
      ...
      for(int i=0; i<dataArray.length; i++){
         int j=0;
         if(buckets[getPlaceValue(curPlace, dataArray[i])][j]==SENTINEL){
            buckets[getPlaceValue(curPlace, dataArray[i])][j] = dataArray[i];
         } 
         else {
            do{
               j++;
            } while(buckets[getPlaceValue(curPlace, dataArray[i])][j]!=SENTINEL);
            buckets[getPlaceValue(curPlace, dataArray[i])][j] = dataArray[i];
         }
      }
   }
